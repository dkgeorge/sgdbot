...
class Strategy(strategy.Strategy):
    ...
    def slot_keypress(self, gox, key):
        ...
        if key == "s":
            TRADE_TYPE = STOP_LOSS
            dialog = NewOrder(STDSCR, gox, curses.color_pair(7), TRADE_TYPE)
            dialog.modal()
            ...
            TRADE_TYPE = START_GAIN 
            dialog = NewOrder(STDSCR, gox, curses.color_pair(6), TRADE_TYPE)
            dialog.modal()
            prc, vol = dialog.do_submit(dialog.price, dialog.volume)
    ...
    def end_bot(self):
        ...
        if self.total_filled:
            ...
            % (self.total_filled - volume_for_fee, trade_asset))  
